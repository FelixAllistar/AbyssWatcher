...
impl LineParser {
    pub fn parse_line(&mut self, line: &str, source: &str) -> Option<CombatEvent> {
        ...
        let event_type = if lower.contains("repaired to")
            || lower.contains("repaired by")
            || lower.contains("boosted to")
    ...
    pub fn parse_notify_line(&mut self, line: &str, source: &str) -> Option<NotifyEvent> {
        ...
        let body = trimmed.split("(notify)").nth(1).map(str::trim)?;
        ...
        EventType::Neut => {
            ...
            if raw_lower.contains("0xffe57f7f") {
                ...
                if lower_body.contains(" neutralized by ") || lower_body.contains(" drained by ") {
                    Some(Direction::Incoming)
                } else if lower_body.contains(" neutralized ")
                    || lower_body.contains(" drained from ")
                    || lower_body.contains(" drained to ")
                {
                    Some(Direction::Outgoing)
                } else {
                    None
...
fn split_amount_body(body: &str) -> Option<(f32, &str)> {
    ...
    if clean_remainder.starts_with("GJ") {
        clean_remainder = clean_remainder
            .strip_prefix("GJ")
            .unwrap_or(clean_remainder)
            .trim();
    }
...
fn split_entities_and_weapon(
    ...
    match event_type {
        EventType::Damage => {
            ...
            if parts
                .last()
                .map(|s| qualities.iter().any(|q| s.to_lowercase().contains(q)))
                .unwrap_or(false)
            {
    ...
    match direction {
        Direction::Outgoing => {
            let prefixes = match event_type {
                ...
                EventType::Repair => vec![
                    "remote armor repaired to ",
                    "remote shield repaired to ",
                    "remote hull repaired to ",
                    "remote armor boosted to ",
                    "remote shield boosted to ",
                    "remote hull boosted to ",
                    "armor repaired to ",
                    "shield repaired to ",
                    "hull repaired to ",
                    "armor boosted to ",
        ...
        Direction::Incoming => {
            let prefixes = match event_type {
                ...
                EventType::Repair => vec![
                    "remote armor repaired by ",
                    "remote shield repaired by ",
                    "remote hull repaired by ",
                    "remote armor boosted by ",
                    "remote shield boosted by ",
                    "remote hull boosted by ",
                    "armor repaired by ",
                    "shield repaired by ",
                    "hull repaired by ",
                    "armor boosted by ",
...
mod tests {
    ...
    fn parses_outgoing_repair() {
        ...
        let event = parser
            .parse_line(line, "LogiPilot")
            .expect("should parse repair");
    ...
    fn parses_outgoing_cap_transfer() {
        ...
        let event = parser
            ...
            .expect("should parse cap transfer");
    ...
    fn parses_incoming_neut() {
        ...
        let _ = parser.parse_line(
            "Session Started: 2026.01.06 21:56:26",
            "I CherryPick Gneiss",
        );
        ...
        let event = parser
            .parse_line(line, "I CherryPick Gneiss")
            .expect("should parse incoming neut");

        assert_eq!(event.amount, 38.0);
        assert_eq!(event.event_type, EventType::Neut);
        assert!(
            event.incoming,
            "Should be incoming neut based on color 0xffe57f7f"
        );
    }
    ...
    fn parses_double_dash_repair() {
        ...
        let event = parser
            .parse_line(line, "Felix")
            .expect("should parse double dash");
    ...
    fn parses_target_with_dash_in_name() {
        ...
        let event = parser
            .parse_line(line, "TestPilot")
            .expect("should parse target with dash");
    ...
    fn parses_damage_without_quality() {
        ...
        let event = parser
            .parse_line(line, "TestPilot")
            .expect("should parse without quality");
    ...
    fn parses_capacitor_failure_notify() {
        ...
        let event = parser
            .parse_notify_line(line, "TestPilot")
            .expect("should parse capacitor failure");
    ...
    fn parses_afterburner_cap_failure() {
        ...
        let event = parser
            .parse_notify_line(line, "TestPilot")
            .expect("should parse afterburner cap failure");
